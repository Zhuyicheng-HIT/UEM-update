...
def main():
    ...
    apply_ema_weights_from_checkpoint(model.model, checkpoint)
    del checkpoint
    smpl = get_smpl()
    for split in ["val"]:
        ...
        for task, seq_name, start_idx in examples:
            for jdx in range(1):
                ...
                imgs = vis_fn(aria_traj=gt_aria_traj_T, verts=gt_verts, faces=smpl.faces, pred_verts=pred_verts)
                seq_name = batch["misc"]["seq_name"][0]
                start_idx, end_idx = (batch["misc"]["start_idx"][0], batch["misc"]["end_idx"][0])
                save_video(
                    imgs[..., ::-1], f"{split}_idx{idx}_{seq_name}_{start_idx}_{end_idx}_{task}_{jdx}", out_dir, fps=10
                )
